...
class vdisperso{
	...
	public:
		...
		void cambiar_nulo(const T & x){
			nulo = x;
			for(typename list<pair<int, T>>::iterator it = coeficientes.begin(); it != coeficientes.end(); it++){
				if(it->second == nulo)
					it = coeficientes.erase(it);
			}
		}
